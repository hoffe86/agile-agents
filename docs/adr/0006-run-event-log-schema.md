...
- **Note (2026-08):** ADR 0009 removed the `testing` agent and ADR 0010 added `code-review`, so the schema's `agent` enum below is out of date as written. The schema decision — JSONL, OTel-inspired event types, dev-lead-emitted, no self-reported cost fields — is unchanged; `references/event-schema.json` carries the current enum.
