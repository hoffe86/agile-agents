...
- **Note (2026-08):** ADR 0009 merged `testing` into `coding`, so the `mid` row below is now `coding`, `infrastructure`, `backlog-manager`. ADR 0010 then split the generalist reviewer back out of the orchestrator, so the `heavy` row's `code-review` is once again an agent that exists, alongside `review` (orchestrator), and `testing-review` there means today's `test-review`. The tiering convention itself is unchanged.
