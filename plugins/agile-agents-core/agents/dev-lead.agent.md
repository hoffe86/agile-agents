...
**Entry condition:** the `TESTS COMPLETE` block from Stage 7 has been received and parsed — or Stage 7 was skipped as IaC-only, in which case skip the test bar too (`infrastructure` already ran its own IaC tests). Deploy-verify below still applies to IaC-only changes.
