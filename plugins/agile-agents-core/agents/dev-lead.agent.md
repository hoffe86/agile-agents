...
- **Stop conditions (mandatory human input):**
  ...
  13. **Git write not yet approved** — before the first commit, push, or PR of the run, stop and ask. Ask **once**, name what will be committed and to which branch, and carry the answer for the rest of the run. Plan approval at Stage 4 is approval of the *plan*, not of writing to the repository.
...
| Branch, commit, push, open or update a PR | `coding`, `testing`, `infrastructure`, or you | **explicit user approval**, asked once |
...
- **Write permissions.** Your `execute` grant covers the orchestration scripts only (`run-event-log`, `cost-budget`, `test-bar-gate`) — no build, no deploy. Git writes and non-production deploys are permitted under the policy table at the end of Stage 10; **completing/merging/closing a PR, force-pushing, rewriting shared history and production deploys are human-only, always**.
