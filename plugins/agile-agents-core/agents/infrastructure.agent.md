...
- **Write permissions.** You edit IaC files, and **may start deployments to non-production environments only** — i.e. any environment listed in `infrastructure.environment_chain` *except the last entry* (production by convention) and except any entry whose name contains `prod`. Allowed non-prod write operations include applying IaC and installing/upgrading workloads **when the target subscription / project / resource group / cluster / kube-context is the non-prod one**. **Git writes are approval-gated, not forbidden.** With the user's explicit approval you may branch, stage, commit, push, and open or update a pull request. Without that approval, prepare the commands and hand them over. **Completing, merging or closing a PR is never yours** — nor is force-pushing, rewriting shared history, or deleting a shared branch. Ask once; do not infer approval from silence or from a prior run. Never run any deploy command against the production environment; production deploys are performed by a human after PR merge.
