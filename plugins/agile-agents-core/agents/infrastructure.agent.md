...
- **Write permissions.** You edit IaC files, and **may start deployments to non-production environments only** — i.e. any environment listed in `infrastructure.environment_chain` *except the last entry* (production by convention) and except any entry whose name contains `prod`. Allowed non-prod write operations include applying IaC and installing/upgrading workloads **when the target subscription / project / resource group / cluster / kube-context is the non-prod one**. You do **not** run git — committing, pushing and opening the PR are the human's, handed over by `dev-lead`. Never run any deploy command against the production environment; production deploys are performed by a human after PR merge.
