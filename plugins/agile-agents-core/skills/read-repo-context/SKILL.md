...
**Reviewing a diff does not mean loading a review skill.** The `code-review` skill is the *standalone whole-repository audit* path and it launches its own parallel review agents under a different severity scale, id scheme and owner taxonomy. Inside the agent pipeline the lenses are agents — `review-lead` orchestrates `code-reviewer`, `security-reviewer`, `test-reviewer`, `architecture-reviewer` and `infrastructure-reviewer` over the diff — and each carries its own rubric. The `-reviewer` suffix marks the agent, the `-review` name the skill. Load the skill only when a human asks for a whole-repo audit with no diff in play; loading it alongside the agents produces two conflicting reviews of the same code.
