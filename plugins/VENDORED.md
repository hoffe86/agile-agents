...
|---|---|---|---|
| polyglot-test-agent | `agile-agents-core` | `github/awesome-copilot/skills/polyglot-test-agent` | Removed upstream (confirmed 404, 2026-08). **Kept** — `testing-practices` routes to it as the cross-language fallback when no language skill is installed, and `test-reviewer` cites it for cross-language test scaffolding. Editing it in place is now allowed; it has no upstream to diverge from. |
